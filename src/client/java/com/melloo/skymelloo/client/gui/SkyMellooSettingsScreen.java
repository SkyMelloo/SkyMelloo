...
public class SkyMellooSettingsScreen extends Screen {
	...
	public enum Tab {
		...
		// Highlighting removed entirely the same day - HP Armor Stand highlighting (the only thing left in it
		// after Ore Highlighting was removed) is gone too, not relocated.
	...
	private static Tab[] visibleTabs() {
		...
		for (Tab tab : Tab.values()) {
			boolean visible = switch (tab) {
				...
				// something inside it.
				case DUNGEONS -> PermissionsManager.has("dungeonInfo") || PermissionsManager.has("chestHighlight") || PermissionsManager.has("itemHighlight") || PermissionsManager.has("mobHighlight") || PermissionsManager.has("killTracker");
	...
	private List<RowFactory> rowsFor(Tab tab) {
		...
		switch (tab) {
			...
			case PARTY -> {
				// Drastically simplified (2026-07-27, "generell kein highlight mehr und komplett umbauen
				...
				rows.add(tip(boolRow("Show Invisible Players", () -> c.showInvisiblePlayers, v -> c.showInvisiblePlayers = v, 0xFF888888), "Makes other invisible players (e.g. from an Invisibility Potion) render normally instead of staying hidden. NOT a glow/Highlighting effect - they're still blocked by walls/line-of-sight like anyone else, just no longer artificially hidden."));
			}
			case HP -> {
				rows.add(headerRow("Distance"));
				rows.add(tip(boolRow("Show Distance", () -> c.highlightShowDistance, v -> c.highlightShowDistance = v, 0xFF55FFFF), "Show distance in blocks next to Highlighting'd mobs/players/items."));
			}
			case FISHING -> {
			...
			case DUNGEONS -> {
				// Chest/Item/Mob highlighting all relocated here from their old Highlighting/Items tabs
				// (2026-07-27, "chest highlight nach dungeons verschieben und chest highlight nennen item
				// highlight auch dahin ... mob highlight auch mob highlighting nach dungeons verschieben") -
				// purely a settings-screen reorg, the underlying scan/render logic (HighlightManager,
				// BlockHighlightRenderer) is unchanged and still works everywhere in-game, not just dungeons.
				if (PermissionsManager.has("chestHighlight")) {
					rows.add(headerRow("Chest Highlight"));
					rows.add(tip(boolRow("Chest Highlight", () -> c.chestHighlightEnabled, v -> c.chestHighlightEnabled = v, 0xFFFFD700), "Highlight chests with a glowing box outline, visible through walls."));
				...
				if (PermissionsManager.has("itemHighlight")) {
					rows.add(headerRow("Item Highlighting"));
					rows.add(tip(boolRow("Item Highlighting", () -> c.itemHighlightEnabled, v -> c.itemHighlightEnabled = v, 0xFF55FF55), "Highlight dropped items with a glowing outline and show their name, visible through walls."));
				...
				// ... generell kein highlight mehr") - the old general "highlight every hostile mob
