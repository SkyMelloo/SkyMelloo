...
/**
 * Decides which entities get the forced-glow ESP treatment and what color they get.
...
public final class EspManager {
	...
	/** Briefly (3s) forces a player's ESP color/glow to orange - called right after you kill them. */
	...
	public static boolean shouldGlow(Entity entity) {
		...
		if (BlockEspRenderer.isChestMarker(entity)) {
			return PermissionsManager.has("chestEsp");
		}
		...
		if (living instanceof Player player) {
			// "playerEsp" is its own permission, split off from the dungeon mob highlight's "esp"
			// (2026-07-27) - toggling one must not toggle the other.
			if (!PermissionsManager.has("playerHighlight") || !config.playerEspEnabled) {
				return false;
			}
		...
		// Only the dungeon current-room mob highlight remains (2026-07-27, "das esp auf monster
	...
	public static boolean isEspTarget(Entity entity) {
		...
		if (entity instanceof Player player) {
			...
			if (!config.playerEspEnabled || !PermissionsManager.has("playerHighlight")) {
				return false;
			}
	...
	private static boolean shouldGlowItem(ItemEntity item, SkyMellooConfig config) {
		if (!config.itemEspEnabled || !PermissionsManager.has("itemEsp")) {
			return false;
		}
	...
	public static Component colorizeName(Player player, Component original) {
		...
		if (!flashing && (!config.playerEspEnabled || !PermissionsManager.has("playerHighlight"))) {
			return original;
		}
